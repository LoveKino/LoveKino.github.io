...
`),
    ...
    code(`stack iteration x 84,084 ops/sec ±6.83% (58 runs sampled)
...
Fastest is stack iteration,recursion`),
    ...
    code(`int recursiveSum(Node *root) {
  if (root == NULL)
    return 0;
...
`),
    ...
    code(`int stackIterateSum(Node *root) {
  vector<Node *> stack;
...
`),
    ...
    list([
        ...
        'In lower-level language which we can control memory, if we know the depth of recursion, we can use an array to simulate stack. We allocate memory at first (this trick will avoid multiple times memory allocating). This kind of stack-simulation will be fast. If we want to rewrite a recursion code for better performance, we can use this trick.'
    ])
]);
